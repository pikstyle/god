...
function Home({ isLeader, textHome, setTextHome, saveHomeContent, user, partiLeader, gameState }) {
    ...
    return ( // Maj le state localement lors du onChange et avec saveHomeContent(textHome) ecrit dans la base de donnée
        <div className={styles.div_home}>
            ...
            {isLeader && gameState?.regne ? <div><input value={textHome} onChange={(e) => setTextHome(e.target.value)}/> <button onClick={() => saveHomeContent(textHome)}>Save</button></div>: null}
        </div>
    )
}
